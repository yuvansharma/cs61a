...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        our_func = scheme_eval(expr.first, env)
        if not expr.rest:
            return scheme_apply(our_func, nil, env)
        python_list = Pair.map(expr.rest, lambda op: scheme_eval(op, env))
        
        return scheme_apply(our_func, python_list, env)
        # END PROBLEM 3
...
def complete_apply(procedure, args, env):
    ...
    if isinstance(val, Unevaluated):
        return scheme_eval(val.expr, val.env, True)############# CHECK THIS BRUH, True)
    else:
        return val
