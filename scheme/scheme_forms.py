...
def do_and_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    #if expressions is nil: # 1 and 2 and #f and 5
    #    return True
    #temp = expressions
    #while not(temp.rest is nil):
        # x = scheme_eval(temp.first, env)
...
def make_let_frame(bindings, env):
    ...
    temp = bindings
    while not(temp is nil):
        validate_form(temp.first, 2, 2)
        names = Pair(temp.first.first, names)
        vals = Pair(scheme_eval(temp.first.rest.first, env), vals) #### ADD TRUE MAYBE
        temp = temp.rest
