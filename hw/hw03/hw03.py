...
def pingpong(n):
    ...
    if n == 1:
        return 1
#    if n%10==8 or num_eights(n) != 0:
 #       return pingpong(n-1)
    else:
        return pingpong(n-1) + find_direction(n)   

def find_direction(n):
    direction = 1
    for i in range(1,n):
...
def count_coins(change):
    ...
    def count_partitions(change, coin):
        ...
        if coin == None:
            ...
            if coin > change:
                coin = next_smaller_coin(coin)
            return count_partitions(change - coin, coin) + count_partitions(change, next_smaller_coin(coin))
